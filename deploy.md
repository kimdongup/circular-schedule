...
Web Push 수신 확인은 `/`의 `푸시 알림 설정`에서 구독한 다음 시스템 알림 테스트로 진행합니다. 알림은 페이지 내부가 아니라 운영체제 알림 센터에 표시됩니다. iPhone과 iPad에서는 사이트를 홈 화면에 추가한 뒤 설치된 PWA에서 알림 권한을 허용해야 합니다.
...
로그인 이메일을 `ADMIN_EMAILS`에 정확히 등록하거나 Supabase SQL Editor에서 다음을 실행합니다.
